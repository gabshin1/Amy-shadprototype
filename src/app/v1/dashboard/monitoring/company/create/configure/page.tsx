...
// Mock integration status - in a real app, this would come from a store/API
const mockIntegrationStatus = {
  email: { connected: true, options: [{ id: "gmail", name: "Gmail", connected: true }] },
  slack: { connected: true, options: [{ id: "slack_app", name: "Slack App", connected: true }] },
}

// Mock Slack channels - in a real app, this would be fetched from Slack API
...
export default function ConfigureWatchlistPage() {
  ...
  const [integrationModalTab, setIntegrationModalTab] = React.useState<string>("slack")
  ...
  const openIntegrationModal = (tab: "slack" | "email") => {
    setIntegrationModalTab(tab)
  ...
  const handleCreateWatchlist = () => {
    ...
    const validEmailAddresses = emailAddresses.filter((email) => email.trim().length > 0)
    ...
    <div className="flex flex-1 flex-col gap-6 p-8 max-w-7xl mx-auto w-full">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Priority Alert Notifications</CardTitle>
              <CardDescription>
                Configure how you want to be notified when priority alerts are triggered
              </CardDescription>
            </CardHeader>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Receive priority alerts via email
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {mockIntegrationStatus.email.connected ? (
                      <Badge variant="outline" className="bg-green-500/10 text-green-600 border-green-200 gap-1.5">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => openIntegrationModal("email")}
                      >
                        Connect
                    ...
                    <Switch
                      ...
                      disabled={!mockIntegrationStatus.email.connected}
                    />
                  </div>
                </div>
                {emailNotifications && mockIntegrationStatus.email.connected && (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Receive priority alerts in Slack
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {mockIntegrationStatus.slack.connected ? (
                      <Badge variant="outline" className="bg-green-500/10 text-green-600 border-green-200 gap-1.5">
                        <CheckCircle2 className="h-3 w-3" />
                        Connected
                      </Badge>
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => openIntegrationModal("slack")}
                      >
                        Connect
                        <ExternalLink className="ml-1.5 h-3 w-3" />
                      </Button>
                    ...
                    <Switch
                      ...
                      disabled={!mockIntegrationStatus.slack.connected}
                    />
                  </div>
                </div>
                {slackNotifications && mockIntegrationStatus.slack.connected && (
                  <div className="space-y-2 pl-8">
                    <div className="space-y-2">
      ...
      <IntegrationModal
        ...
        defaultTab={integrationModalTab}
      />
